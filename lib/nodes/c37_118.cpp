...
void C37_118::Output::prepare(NodeDirection &out) {
  ...
  for (;;) {
    ...
    case GET_CONFIG1:
      fmt::println("received get config1 command");
      send(Frame::Message::make<Frame::Type::CONFIG1>(config));
      continue;
    case GET_CONFIG2:
      fmt::println("received get config2 command");
      send(Frame::Message::make<Frame::Type::CONFIG2>(config));
      continue;
    case DATA_START:
      fmt::println("received get data start command");
      data_stream = true;
      return;
    default:
      fmt::println("received unhandled command");
      continue;
    }
  }
}
